...
for shape_key, shape_info in pulse_shapes.items():
    ...
    # Create sequence
    sequence = (ShapedPulseSequence(f"{shape_info['name']} Echo")
        .add_shaped_pulse(
            flip_angle=np.pi/2,
...
ax.set_title('Comparison of Pulse Shapes - Sy Component', fontsize=13, fontweight='bold')
...
ax.set_title('Comparison of Pulse Shapes - Sx Component', fontsize=13, fontweight='bold')
ax.legend(fontsize=10, loc='best')
ax.grid(True, alpha=0.3)
...
efficiencies = [np.max(np.abs(results[key]['sy'])) * 100
                for key in pulse_shapes.keys()]
colors = [info['color'] for info in pulse_shapes.values()]

bars = ax.bar(range(len(shape_names)), efficiencies, color=colors, alpha=0.7, edgecolor='black')
...
plt.suptitle('Pulse Shapes Comparison - NMR/ESR Simulations',
