...
class SpinEchoGUI:
    ...
    def create_simulation_controls(self, parent):
        ...
        self.tau1_scale = ttk.Scale(frame, from_=0.1, to=20.0, variable=self.tau1,
                                   orient=tk.HORIZONTAL, length=200,
                                   command=self._create_formatted_callback(self.tau1, self.on_parameter_change))
        ...
        self.tau2_scale = ttk.Scale(frame, from_=0.1, to=20.0, variable=self.tau2,
                                   orient=tk.HORIZONTAL, length=200,
                                   command=self._create_formatted_callback(self.tau2, self.on_parameter_change))
    ...
    def update_plot_title(self):
        ...
        if hasattr(self, 'ax'):
            if self.sequence_type.get() == "2pulse":
                self.ax.set_title('2-Pulse Spin Echo Simulation', fontsize=10, fontweight='bold')
            else:
                self.ax.set_title('3-Pulse Stimulated Echo Simulation', fontsize=10, fontweight='bold')
    ...
    def _calculate_worker(self):
        ...
        try:
            ...
            self.root.after(0, self._update_progress, "Processing results and applying distribution weighting...")
